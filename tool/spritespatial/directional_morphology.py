...
def apply_directional_interval(
    ...
    front_scale = 1.0 + max(0.0, rule.forward_bias) * 0.45
    back_scale = 1.0 + max(0.0, rule.backward_bias) * 0.75
    ...
    front_extent = max(dz * 0.55, half * max(0.30, front_scale) * edge_fullness)
    back_extent = max(dz * 0.55, half * max(0.30, back_scale) * edge_fullness)
